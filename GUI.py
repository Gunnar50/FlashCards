...
class TextField:
  def __init__(
    ...
    self.character = 'a'
    self.character_render = pygame.font.SysFont(
  ...
  def draw(self) -> None:
    ...
    if self.text != '':
      ...
      self.text_render = self.sys_font.render(
        self.placeholder, True, (100, 100, 100)
      )

    self.input_rect = pygame.Rect(
      self.x,
      self.y,
      self.width + 2 * self.character_render.get_width(),
      ...
      if self.timer > 30:
        self.bar = pygame.draw.line(
          ...
          (
            ...
            8 + self.y + self.character_render.get_height(),
          ),
          2,
        )
        if self.timer == 60:
...
class Button:
  def __init__(
    ...
    self.screen = screen
    self.x, self.y = x, y
    self.button_colour, self.button_outline_colour = (
      button_colour,
      button_outline_colour,
    )
...
class Tab:
  ...
  def draw_text(self) -> None:
    for i, text in enumerate(self.options):
      tab = Button(
        ...
        button_outline_colour=self.outline_colour,
        font=self.font,
  ...
  def is_clicked(self, event) -> None:
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
      for tab in self.tabs_list:
        if (
          tab.x <= mx <= tab.x + tab.width and tab.y <= my <= tab.y + tab.height
        ):
  ...
  def draw(self) -> None:
    ...
    if self.toggle:
      pygame.draw.rect(
        self.screen,
        self.colour,
        (self.x, self.y, self.width / 2, self.height),
        0,
        border_top_left_radius=self.round_border,
        border_bottom_left_radius=self.round_border,
      )
      ...
      pygame.draw.rect(
        self.screen,
        self.colour,
        (self.x + self.width / 2, self.y, self.width / 2, self.height),
        0,
        border_top_right_radius=self.round_border,
        border_bottom_right_radius=self.round_border,
      )
    ...
    draw_x_right = (
      self.x
      + (self.width / 2 - text_right.get_width() / 2)
...
class CheckBox:
  ...
  def draw_font(self) -> None:
    ...
    self.render_text = font.render(self.text, True, self.font_colour)
    self.draw_y = (
      self.y + (self.box_size / 2) - (self.render_text.get_height() / 2)
    )
    if self.right_text:
  ...
  def draw(self) -> None:
    if not self.radio:
      ...
      pygame.draw.rect(
        self.screen,
        self.box_colour,
        (self.x, self.y, self.box_size, self.box_size),
      )
      if self.checked:
        ...
        pygame.draw.line(
          ...
          self.box_colour
          if self.selected_colour is not None
          else self.box_outline,
          (self.x + self.box_size / 4, self.y + self.box_size / 2),
          (
            self.x + (self.box_size / 2) - self.box_size / 10,
            self.y + 3 * (self.box_size / 4),
          ),
          3,
        )
        pygame.draw.line(
          ...
          (self.x + 3 * (self.box_size / 4), self.y + self.box_size / 4),
          3,
        )
    else:
      # radio button
  ...
  def is_clicked_2(self, event) -> None:
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
      if (
        self.x <= mx <= self.x + self.box_size
        and self.y <= my <= self.y + self.box_size
      ):
      ...
      if self.text_wrap_obj is not None:
        if self.right_text:
          if (
            ...
            <= self.x
            + self.box_size
            + self.text_wrap_obj.wrap_text.get_width()
            + self.spacing
            and self.y <= my <= self.y + self.box_size
          ):
            self.checked = not self.checked
          ...
          if (
            self.x - self.text_wrap_obj.wrap_text.get_width() - self.spacing
            <= mx
  ...
  def is_clicked_2(self, event) -> None:
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
      for button in self.radio_button_list:
        if (
          button.x <= mx <= button.x + button.box_size
          and button.y <= my <= button.y + button.box_size
        ):
          button.checked = True
          for box in self.radio_button_list:
            if box != button:
        ...
        if self.wrap_text:
          if self.right_text:
            if (
              ...
              <= button.x
              + button.box_size
              + button.text_wrap_obj.wrap_text.get_width()
              + button.spacing
              and button.y <= my <= button.y + button.box_size
            ):
              button.checked = True
...
class DropDown:
  ...
  def draw_font(self) -> None:
    ...
    self.draw_x = self.x + (
      self.box_width / 2 - self.render_text.get_width() / 2
    )
    self.draw_y = self.y + (
  ...
  def create_options(self) -> None:
    ...
    self.render_options = []
    for i, option in enumerate(self.options):
      self.options_rect.append(
        [
          self.box_colour,
          pygame.Rect(
            self.x,
            self.y + self.box_height + (self.box_height * i),
            self.box_width,
            self.box_height,
          ),
        ]
  ...
  def draw(self) -> None:
    ...
    pygame.draw.rect(
      self.screen,
      self.box_outline_colour,
      (
        self.x - self.outline_width,
        self.y - self.outline_width,
        self.box_width + (2 * self.outline_width),
        self.box_height + (2 * self.outline_width),
      ),
    )
    pygame.draw.rect(
      self.screen,
      self.box_colour,
      (self.x, self.y, self.box_width, self.box_height),
    )
  ...
  def is_clicked(self, event) -> None:
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
      if self.opened:
        self.opened = False
        for rect, i, render in zip(
          self.options_rect,
          range(len(self.render_options)),
          self.render_options,
        ):
          if (
            rect[1].x <= mx <= rect[1].x + self.box_width
            and rect[1].y <= my <= rect[1].y + self.box_height
          ):
            self.selected = True
            self.render = render
        ...
        self.x <= mx <= self.x + self.box_width
        and self.y <= my <= self.y + self.box_height
      ):
