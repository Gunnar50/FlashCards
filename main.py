      ...
      if event.type == pygame.MOUSEBUTTONDOWN:
        if event.button == 3:
          if self.is_question:
            ...
            self.progress = (
              f'{len(self.correct_questions)} / {len(self.flash_cards)}'
            )
            print(self.progress)
          else:
    ...
    if self.flash_cards[-1].correct_answer:
      ...
      if len(self.flash_cards) > 50:
        index = random.randint(
          len(self.flash_cards) - len(self.flash_cards) // 4,
          len(self.flash_cards) - 4,
        )

      else:
        index = random.randint(
          len(self.flash_cards) // 2, len(self.flash_cards) - 4
        )
